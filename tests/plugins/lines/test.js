...
describe('Prism Lines Plugin', function () {
	...
	it('- should split lines correctly', function () {
		assert.equal(highlightedMarkup(`/* a \n b */`, 'javascript'), [
			'<div class="code-line"><span class="token comment">/* a \n</span></div>',
			'<div class="code-line"><span class="token comment"> b */</span></div>'
		].join(''));

		assert.equal(highlightedMarkup(`var\n/* a \n b */\nbar\n=\n\n0;`, 'javascript'), [
			'<div class="code-line"><span class="token keyword">var</span>\n</div>',
			'<div class="code-line"><span class="token comment">/* a \n</span></div>',
			'<div class="code-line"><span class="token comment"> b */</span>\n</div>',
			'<div class="code-line">bar\n</div>',
			'<div class="code-line"><span class="token operator">=</span>\n</div>',
			'<div class="code-line">\n</div>',
			'<div class="code-line"><span class="token number">0</span><span class="token punctuation">;</span></div>'
		].join(''));


		assert.equal(processMarkup(`foo\nfoo<x>foo\nfoo</x>`, 'javascript'), [
			'<div class="code-line">foo\n</div>',
			'<div class="code-line">foo<x>foo\n</x></div>',
			'<div class="code-line"><x>foo</x></div>'
		].join(''));
	});

	it('- should handle zero-width elements correctly', function () {
		assert.equal(processMarkup(`<a></a>\n<a></a>\n<a></a>`, 'javascript'), [
			'<div class="code-line"><a></a>\n</div>',
			'<div class="code-line"><a></a>\n</div>',
			'<div class="code-line"><a></a></div>'
		].join(''));

		assert.equal(processMarkup(`<x><y>\n</y></x>\n<x><y>\n</y><z></z></x>\n`, 'javascript'), [
			'<div class="code-line"><x><y>\n</y></x></div>',
			'<div class="code-line">\n</div>',
			'<div class="code-line"><x><y>\n</y></x></div>',
			'<div class="code-line"><x><z></z></x>\n</div>'
		].join(''));

		assert.equal(processMarkup(`<x><y><x/></y><z></z></x><y></y>`, 'javascript'), [
			'<div class="code-line"><x><y><x></x></y><z></z></x><y></y></div>'
		].join(''));
	});

	it('- should not create empty last lines', function () {
		assert.equal(processMarkup(`foo`, 'javascript'), [
			'<div class="code-line">foo</div>'
		].join(''));
