...
function docsRemoveDate() {
	return gulp.src('docs/*.html')
		.pipe(replace(
			/<footer>\s*.*\s*<\/footer>/,
			'<footer>Generated by <a href="https://github.com/jsdoc3/jsdoc">JSDoc 3.5.5</a> using the Minami theme.</footer>'
		))
		.pipe(gulp.dest('docs/'));
}
function docsRemoveExcessFiles() {
	return gulp.src('docs/fonts', { read: false }).pipe(clean());
}

gulp.task('docs', gulp.series(docsClean, docsCreate, gulp.parallel(docsRemoveDate, docsRemoveExcessFiles)));


gulp.task('default', gulp.parallel('components', 'components-json', 'plugins', 'build', 'docs'));
